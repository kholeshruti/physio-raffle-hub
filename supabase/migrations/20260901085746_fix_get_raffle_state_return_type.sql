/*
# Fix get_raffle_state return type for PostgREST compatibility

PostgREST handles RETURNS TABLE(...) more reliably than scalar json
for .rpc() calls from the Supabase JS client.
*/
...
begin
  ...
  select max(batch_number) into v_max_batch from tickets;
  ...
  select count(*) into v_sold_count from tickets where status = 'sold';

  select coalesce(json_object_agg(ticket_number, status), '{}'::json)
  into v_taken
